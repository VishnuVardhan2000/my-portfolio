...
function NeonCard({ children, className = "", style = {} }) {
  ...
  return (
    <div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave}
      className={`neon-spotlight ${className}`} style={style}>
      {children}
    </div>
  );
}
  ...
  return (
    <>
      <style>{`
        ...
        .neon-spotlight > * { position: relative; z-index: 1; }
      `}</style>
      ...
      <main className="min-h-screen bg-[#0a0a12] text-white overflow-x-hidden">
        ...
        <div className="max-w-5xl mx-auto px-6 pt-16 pb-24 relative z-10">
          ...
          {/* ── Currently ── */}
          <NeonCard className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 bg-white/[0.02] border border-white/[0.07] rounded-2xl p-6">
            <div>
