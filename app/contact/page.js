...
function RPGFrame({ children, title, accent = "0,255,136" }) {
  ...
  return (
    ...
    >
      <span className="absolute top-[-1px] left-[-1px] w-3 h-3" style={{ borderTop: `2px solid ${c}0.8)`, borderLeft: `2px solid ${c}0.8)`, borderRadius: "2px 0 0 0" }} />
      <span className="absolute top-[-1px] right-[-1px] w-3 h-3" style={{ borderTop: `2px solid ${c}0.8)`, borderRight: `2px solid ${c}0.8)`, borderRadius: "0 2px 0 0" }} />
      <span className="absolute bottom-[-1px] left-[-1px] w-3 h-3" style={{ borderBottom: `2px solid ${c}0.8)`, borderLeft: `2px solid ${c}0.8)`, borderRadius: "0 0 0 2px" }} />
      <span className="absolute bottom-[-1px] right-[-1px] w-3 h-3" style={{ borderBottom: `2px solid ${c}0.8)`, borderRight: `2px solid ${c}0.8)`, borderRadius: "0 0 2px 0" }} />
      {title && (
        <div className="absolute -top-3.5 left-6">
          <span className="font-mono text-[11px] font-bold tracking-widest px-3 py-0.5 rounded" style={{ color: `rgba(${accent},1)`, background: "#0a0a12", border: `1px solid ${c}0.25)` }}>
            // {title}
          </span>
        </div>
      )}
...
function TerminalInput({ label, name, value, onChange, type = "text", required, accent = "0,255,136" }) {
    ...
    <>
      ...
      <section className="max-w-3xl mx-auto px-4 pt-8 pb-24 space-y-10">
        <div className="text-center space-y-3">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white" style={{ fontFamily: "var(--font-syne)", letterSpacing: "0.04em" }}>
            Send a Message
          </h1>
          <p className="text-gray-400 text-sm font-mono">Open to work, collabs, and side quests. ▌</p>
        </div>

        <RPGFrame title="TRANSMIT_MESSAGE.EXE" accent="0,255,136">
          <form onSubmit={handleSubmit} className="space-y-5 pt-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <TerminalInput label="Name" name="name" value={form.name} onChange={handleChange} required accent="0,255,136" />
            ...
            <TerminalTextarea label="Message" name="message" value={form.message} onChange={handleChange} accent="74,222,128" />

            {status === "success" && (
              <div className="font-mono text-sm rounded-lg px-4 py-3" style={{ background: "rgba(0,255,136,0.08)", border: "1px solid rgba(0,255,136,0.25)", color: "#00ff88" }}>
                ✓ MESSAGE TRANSMITTED SUCCESSFULLY — I will respond within 24hrs.
              </div>
            )}

            {status === "error" && (
              <div className="font-mono text-sm rounded-lg px-4 py-3" style={{ background: "rgba(236,72,153,0.08)", border: "1px solid rgba(236,72,153,0.25)", color: "rgb(236,72,153)" }}>
            ...
            <button
              ...
              style={{
                background: status === "sending" ? "rgba(0,255,136,0.1)" : "rgba(0,255,136,0.12)",
                border: "1px solid rgba(0,255,136,0.35)",
                color: "#00ff88",
                boxShadow: status === "sending" ? "none" : "0 0 20px rgba(0,255,136,0.08)",
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(0,255,136,0.2)"; e.currentTarget.style.boxShadow = "0 0 30px rgba(0,255,136,0.2)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = status === "sending" ? "rgba(0,255,136,0.1)" : "rgba(0,255,136,0.12)"; e.currentTarget.style.boxShadow = status === "sending" ? "none" : "0 0 20px rgba(0,255,136,0.08)"; }}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3">
            {SOCIALS.map((s, i) => (
              <a
                ...
                onMouseEnter={(e) => { e.currentTarget.style.background = `rgba(${s.color},0.06)`; e.currentTarget.style.borderColor = `rgba(${s.color},0.4)`; e.currentTarget.style.boxShadow = `0 0 16px rgba(${s.color},0.1)`; }}
