...
export default function MusicPlayer() {
  ...
  // ── Canvas page effects: particles + aura + shockwaves ──────
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Shockwave rings on beat drops
      if (isPlaying && bass > 0.52 && Math.random() > 0.55) {
      ...
      shockwavesRef.current = shockwavesRef.current.filter((s) => s.opacity > 0.012);
      shockwavesRef.current.forEach((s) => {
        ctx.beginPath();
        ctx.arc(s.x, s.y, s.r, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(${r},${g},${b},${s.opacity})`;
        ctx.lineWidth = Math.max(0.5, (1 - s.r / 700) * 2.5);
        ctx.stroke();
        s.r += 5 + bass * 10;
        s.opacity *= 0.92;
      });

      // Dispatch beat event for About page glitch effect
      const now = Date.now();
      if (now - lastEventRef.current > 50) {
        window.dispatchEvent(
          new CustomEvent("bgm-beat", {
      ...
      <div className="relative" ref={popupRef}>
        ...
        >
          ...
          {isPlaying ? (
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={`rgba(${c},0.8)`} strokeWidth="2">
        ...
        {open && (
          <div
            className="absolute right-0 mt-2 rounded-xl overflow-visible"
            style={{
              width: "262px",
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="mb-3">
                <div
                  className="w-full h-1 rounded-full cursor-pointer mb-1"
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: `0 0 6px rgba(${c},0.6)`,
                      transition: "width 0.3s linear",
                    }}
                  />
                </div>
                <div className="flex justify-between">
                  <span className="font-mono text-[9px]" style={{ color: `rgba(${c},0.4)` }}>
                    {fmt(currentTime)}
                  </span>
                  <span className="font-mono text-[9px]" style={{ color: `rgba(${c},0.4)` }}>
                    {fmt(duration)}
              ...
              <div className="flex items-center justify-center gap-3 mb-3">
                <button
                  onClick={() => switchTrack((trackIdx - 1 + TRACKS.length) % TRACKS.length)}
                  style={{ color: `rgba(${c},0.7)`, background: "none", border: "none", cursor: "pointer", fontSize: "14px" }}
                >
                  ⏮
                </button>
                <button
                ...
                <button
                  onClick={() => switchTrack((trackIdx + 1) % TRACKS.length)}
                  style={{ color: `rgba(${c},0.7)`, background: "none", border: "none", cursor: "pointer", fontSize: "14px" }}
                >
                  ⏭
                </button>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <input
                  ...
                  style={{ accentColor: `rgba(${c},1)`, height: "3px" }}
                />
                <span className="font-mono text-[9px]" style={{ color: `rgba(${c},0.4)` }}>
              ...
              <div className="border-t pt-2" style={{ borderColor: `rgba(${c},0.1)` }}>
                <p className="font-mono text-[8px] tracking-widest uppercase mb-1.5" style={{ color: `rgba(${c},0.35)` }}>
                  Queue
                </p>
                  ...
                  {TRACKS.map((track, i) => (
                    ...
                    >
                      ...
                      {i === trackIdx && isPlaying && (
                        <span className="font-mono text-[8px]" style={{ color: `rgba(${c},0.7)` }}>
                          ▶
                        </span>
                      )}
                    </button>
