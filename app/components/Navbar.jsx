...
function TerminalLabel() {
        ...
        if (i <= 0) {
          ...
          const typeInterval = setInterval(() => {
            ...
            if (j >= full.length) {
              ...
              timeout = setTimeout(deleteThenRetype, 3000);
            }
          }, 80);
        }
      }, 60);
  ...
  const body = text.slice(0, -1);   // "termina"
  const last = text.slice(-1);      // "l" (or whatever last char is while typing)

  return (
    <span className="font-mono text-sm font-medium tracking-wide inline-flex items-center gap-0">
      {/* All chars except last — white */}
      <span style={{ color: "rgba(255,255,255,0.65)" }}>{body}</span>

      {/* Last char — neon green */}
      {last && (
        <span style={{
          color: "#4ade80",
          textShadow:
            "0 0 4px rgba(74,222,128,1), 0 0 10px rgba(74,222,128,0.8), 0 0 22px rgba(74,222,128,0.5)",
        }}>
          {last}
        </span>
      )}
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 bg-[#0a0a12]/80 backdrop-blur-md border-b border-white/[0.06]">
      ...
      <style>{`
        ...
        /* ── Cursor blink (fallback if globals.css not loaded yet) ── */
